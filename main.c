...
int main(void)
{
    ...
    int fd1 = GPIO_OpenAsOutput(RED1, GPIO_OutputMode_PushPull, GPIO_Value_High);
    if (fd1 == -1) {
        Log_Debug(
    ...
    int fd2 = GPIO_OpenAsOutput(GREEN2, GPIO_OutputMode_PushPull, GPIO_Value_High);
    if (fd2 == -1) {
        Log_Debug(
            "Error opening GPIO: %s (%d). Check that app_manifest.json includes the GPIO used.\n",
            strerror(errno), errno);
    ...
    int fd3 = GPIO_OpenAsOutput(BLUE3, GPIO_OutputMode_PushPull, GPIO_Value_High);
    if (fd3 == -1) {
        Log_Debug(
            "Error opening GPIO: %s (%d). Check that app_manifest.json includes the GPIO used.\n",
            strerror(errno), errno);
    ...
    int fd4 = GPIO_OpenAsOutput(RED4, GPIO_OutputMode_PushPull, GPIO_Value_High);
    if (fd4 == -1) {
        Log_Debug(
            "Error opening GPIO: %s (%d). Check that app_manifest.json includes the GPIO used.\n",
            strerror(errno), errno);
    ...
    int fd5 = GPIO_OpenAsOutput(GREEN4, GPIO_OutputMode_PushPull, GPIO_Value_High);
    if (fd5 == -1) {
        Log_Debug(
            "Error opening GPIO: %s (%d). Check that app_manifest.json includes the GPIO used.\n",
            strerror(errno), errno);
    ...
    while (true) {
        ...
        GPIO_SetValue(fd5, GPIO_Value_Low);
        nanosleep(&sleepTime, NULL);
        GPIO_SetValue(fd4, GPIO_Value_High);
        GPIO_SetValue(fd5, GPIO_Value_High);
        nanosleep(&sleepTime, NULL);
    }
}